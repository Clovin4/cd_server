...
Infrastructure-as-code for a DigitalOcean lab environment, currently configured to provision the four-machine cluster required by [Kubernetes the Hard Way](https://github.com/kelseyhightower/kubernetes-the-hard-way).

Terraform manages the VPC and droplets. Ansible runs a production-grade server baseline on each machine after provisioning. GitHub Actions orchestrates the whole pipeline on push to `main`.
...
└── playbooks/
    └── setup.yml           # Ansible — server hardening baseline
```
...
- **SSH hardening** — written as a drop-in to `sshd_config.d/` so it survives package updates. Key settings: keys only (no passwords), `MaxAuthTries 3`, `AllowUsers deploy`, idle session timeout
- **UFW firewall** — default-deny inbound; allows SSH (22), HTTP (80), HTTPS (443)
- **Fail2ban** — bans IPs after 3 failed SSH attempts within 10 minutes; uses `ufw` as the ban backend and `systemd` as the log backend (required on Debian 12)
